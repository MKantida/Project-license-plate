...
$datenow = date('Y-m-d');
//เวลาที่บันทึก
$queryworkio = "SELECT MAX(workdate) as lastdate, workin, workout FROM `licenseplate` 
WHERE idnumber=$idnumber AND workdate='$datenow' ";
$resultio = mysqli_query($con, $queryworkio) or die 
("Error in query: $queryworkio " . mysqli_error($con));
...
<form action="save.php" method="post" enctype="multipart/form-data" name="upfile" id="upfile">
  ...
  <h2>บันทึกการรับนักเรียน</h2>
          ...
          <div class="input-group">
          <label for="idnumber"><b>เวลาเข้า</b></label>
          ...
          <div class="input-group">
            <button type="submit" onclick="alert('บันทึกการรับนักเรียนสำเร็จ')" class="btn">บันทึก</button>
        </div>
</form>
