			...
			if count == (VERIF-1):
				...
				if (license[VERIF-1] != "" ):
					...
					if numlicense == license[VERIF-1] and fetures > feture+5 and point_plate > point: #fetures > fetures+5 เวลาหลัง เทียบกับ เวลาแรก+5
						...
						print("\nInspection time = " + timestamp) ###########################################################โชว์วันเวลา
						font = ImageFont.truetype('Sarun_ThangLuang.ttf', 80)
						img_pil = Image.fromarray(Scene)
						draw = ImageDraw.Draw(img_pil)
						draw.text((ptLowerLeftTextOriginX, ptLowerLeftTextOriginY-50),  numlicense, font = font, fill = (0, 255, 255))
						Scene = np.array(img_pil)
						cv2.imshow("Scene", Scene)            # show scene image โชว์ภาพที่จับได้
					...
					# เพิ่มข้อมูล
							with connection:
								...
								# Create a new record
									sql = "SELECT * FROM `uploadfile` WHERE carnumber = '{}'".format(licPlate.strChars)
									cursor.execute(sql)
									m=cursor.fetchall()
									if not m:
										print('emtry')
										...
										for i in m:
											ty = "INSERT INTO `licenseplate` (`idnumber`, `namestudent`, `carnumber`, `workdate`,`workin`, `carpicture`, `nameparent`) VALUES (%s, %s, %s, %s, %s, %s, %s)"
											cursor.execute(ty, (str(i['idnumber']), str(i['namestudent']) , str(i['carnumber']), timestamp,timestamp, str(i['carpicture']), str(i['nameparent'])  ))
